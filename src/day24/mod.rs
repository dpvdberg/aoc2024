...
impl Wiring {
    fn solve_z(&self) -> u64 {
        let all_wires = self.connections.iter()
        ...
        let wire_bools = all_wires.iter()
            .map(|w| (w, Bool::new_const(&ctx, w.clone())))
            .collect::<HashMap<_, _>>();

        // Set initial values
        for (wire, value) in &self.initial_values {
            let bool_var = wire_bools.get(&wire).unwrap();
        ...
        // Find output
        let z_wires = wire_bools.keys().filter(|k| k.starts_with("z")).sorted().collect::<Vec<_>>();
        let z_vars = z_wires.iter().map(|k| wire_bools.get(k.clone()).unwrap()).collect::<Vec<_>>();

        // Convert z bools to bitvec
        let mut bitvec = ast::BV::from_u64(&ctx, 0, 1);
        for (i, b) in z_vars.iter().enumerate() {
            // Turn each Bool into a 1-bit BV: true => 1, else => 0
            let bit = b.ite(&ast::BV::from_u64(&ctx, 1, 1), &ast::BV::from_u64(&ctx, 0, 1));
            if i == 0 {
                bitvec = bit;
            } else {
                bitvec = bitvec.concat(&bit); // Concatenate new bit to the left
            }
        }
        ...
        if solver.check() == z3::SatResult::Sat {
            ...
            let result = model.eval(&output, true).unwrap();
            println!("Solved number: {}", result.as_u64().unwrap());
...
fn parse_input(input: &str) -> Wiring {
    ...
    let initial_values = raw_initial_values.trim().lines()
    ...
    let connections = re.captures_iter(gates.trim())
        .map(|c| c.extract())
