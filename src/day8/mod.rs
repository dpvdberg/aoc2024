...
fn parse_input(input: &str) -> HashMap<char, Vec<Vector2<i32>>> {
    ...
    for y in 0..lines.len() {
        for x in 0..lines[y].len() {
            ...
            if char != '.' {
                map.entry(char).or_insert_with(Vec::new).push(Vector2::new(x as i32, y as i32));
            }
        }
    }
...
fn find_antinodes_directional(a : Vector2<i32>, b : Vector2<i32>, resonant: bool, bounds: (usize, usize)) -> HashSet<Vector2<i32>> {
...
fn find_antinodes(antennas: &Vec<Vector2<i32>>, resonant: bool, bounds: (usize, usize)) -> HashSet<Vector2<i32>> {
    let mut antinodes = HashSet::<Vector2<i32>>::new();
...
fn count_unique_antinodes(antennas: &HashMap<char, Vec<Vector2<i32>>>, resonant: bool, bounds: (usize, usize)) -> usize {
    antennas.values()
